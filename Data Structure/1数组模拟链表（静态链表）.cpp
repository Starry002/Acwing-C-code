...
int main()
{
    ...
    while(m--)
    {
        ...
        if(op=='I') cin >> k >> x , add(k-1,x);			//由于数组从 0 开始存储 ， 因此第 k 个数的下标为 k-1 ！！！ 
    }
...
int main()
{
    ...
    {
        cin >> op;
        if(op == "L") cin >> x , addr(0,x);
        if(op == "R") cin >> x , faddr(l[1],x);
		if(op == "D") cin >> k , remove(k+1);						//由于下标 idx 从 2 开始 ，因此，第 k 个数字的下标是 k+1
        if(op == "IL") cin >> k >> x , addr(l[k+1],x);				//由于下标 idx 从 2 开始 ，因此，第 k 个数字的下标是 k+1
		if(op == "IR") cin >> k >> x , addr(k+1,x);					//由于下标 idx 从 2 开始 ，因此，第 k 个数字的下标是 k+1
    }
